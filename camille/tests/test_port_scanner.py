...
from camille.modules.port_scanner import _probe_port, scan_ports
from camille.core.models import PortState
...
class TestProbePort:
    def test_open_port_returns_result(self):
        ...
        with patch("camille.modules.port_scanner.socket.create_connection", return_value=mock_sock):
    ...
    def test_closed_port_returns_none(self):
        with patch(
            "camille.modules.port_scanner.socket.create_connection",
...
class TestScanPorts:
    def test_scan_returns_only_open(self):
        def fake_probe(host, port):
            if port in (80, 443):
                from camille.core.models import PortResult
                return PortResult(port=port, state=PortState.OPEN, service="http")
            return None

        with patch("camille.modules.port_scanner._probe_port", side_effect=fake_probe):
